...
class HybridClusteringAnalyzer:
    ...
    def __init__(self, config: Dict = None, data_source: str = 'csv'):
        ...
        """
        self.config = config or MODELS_CONFIG.get('clustering', {})
    ...
    def load_account_data(self, username: str, file_types: List[str] = ['clean']) -> pd.DataFrame:
        """
        Carga datos de una cuenta específica con deduplicación automática.
        
        Args:
            username (str): Nombre de la cuenta (ej: 'BCPComunica')
            file_types (List[str]): Tipos de archivos a cargar ['clean', 'metricas']
        
        Returns:
            pd.DataFrame: Datos cargados y deduplicados
        """
        print(f"📊 Cargando datos para {username}...")
        
        if self.data_source == 'duckdb' and DUCKDB_AVAILABLE:
            return self._load_from_duckdb(username)
        else:
            return self._load_from_csv(username, file_types)
    
    def _load_from_csv(self, username: str, file_types: List[str]) -> pd.DataFrame:
        """Carga datos desde archivos CSV."""
        dfs = []
        data_dir = Path('data')
        
        for file_type in file_types:
            file_path = data_dir / f"{username}_{file_type}.csv"
            if file_path.exists():
                df = pd.read_csv(file_path)
                df['source_file'] = file_type
                dfs.append(df)
                print(f"   ✅ Cargado {file_path.name}: {len(df)} registros")
            else:
                print(f"   ⚠️  No encontrado: {file_path.name}")
        
        if not dfs:
            raise FileNotFoundError(f"No se encontraron archivos para {username}")
        
        # Combinar datasets
        combined_df = pd.concat(dfs, ignore_index=True)
        
        # Deduplicación avanzada (última medición por tweet)
        deduplicated_df = self._deduplicate_tweets(combined_df)
        
        print(f"   🔄 Deduplicación: {len(combined_df)} → {len(deduplicated_df)} registros")
    ...
    def _deduplicate_tweets(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Deduplicación avanzada de tweets basada en fecha y contenido.
        ...
        """
        # Asegurar que timestamp existe para ordenamiento
        if 'timestamp' not in df.columns and 'fecha_publicacion' in df.columns:
            df['timestamp'] = pd.to_datetime(df['fecha_publicacion'])
        
        # Ordenar por timestamp y mantener la última medición
        df_sorted = df.sort_values('timestamp', na_position='first')
        
        # Deduplicar por fecha_publicacion + contenido
        deduplicated = df_sorted.drop_duplicates(
            subset=['fecha_publicacion', 'contenido'], 
            keep='last'
    ...
    def run_clustering_analysis(self, username: str, features: List[str] = None,
        ...
        # K-Means
        kmeans_params = custom_params.get('kmeans', self.config.get('kmeans', {'n_clusters': 5}))
        kmeans = KMeans(**kmeans_params, random_state=42)
        df['cluster_kmeans'] = kmeans.fit_predict(X_scaled)
        
        clustering_results['kmeans'] = {
            'model': kmeans,
            'labels': df['cluster_kmeans'].values,
            'params': kmeans_params,
            'n_clusters': len(np.unique(df['cluster_kmeans']))
        }
        ...
        self._generate_visualizations(df, X_scaled, clustering_results, features, username)
        
        # 8. Análisis de clusters
        cluster_analysis = self._analyze_clusters(df, features, username)
    ...
    def _generate_visualizations(self, df: pd.DataFrame, X_scaled: np.ndarray, 
                               clustering_results: Dict, features: List[str], username: str):
        """Genera visualizaciones completas del clustering."""
        ...
        if len(features) >= 2:
            ...
            # K-Means
            ax1 = axes[0]
            scatter = ax1.scatter(df[features[0]], df[features[1]], 
                                c=df['cluster_kmeans'], cmap='viridis', alpha=0.7)
            ax1.set_xlabel(features[0])
            ax1.set_ylabel(features[1])
            ax1.set_title(f'K-Means Clustering - {username}')
            plt.colorbar(scatter, ax=ax1)
            
            # DBSCAN
            ax2 = axes[1]
            scatter = ax2.scatter(df[features[0]], df[features[1]], 
                                c=df['cluster_dbscan'], cmap='viridis', alpha=0.7)
            ax2.set_xlabel(features[0])
            ax2.set_ylabel(features[1])
            ax2.set_title(f'DBSCAN Clustering - {username}')
        ...
        if len(features) > 2:
            ...
            ax1.set_title(f'K-Means Clustering (PCA) - {username}')
            plt.colorbar(scatter, ax=ax1)
            ...
            ax2.set_title(f'DBSCAN Clustering (PCA) - {username}')
            plt.colorbar(scatter, ax=ax2)
            
            plt.tight_layout()
            plt.show()
    
    def _analyze_clusters(self, df: pd.DataFrame, features: List[str], username: str) -> Dict:
        ...
        for algorithm in ['kmeans', 'dbscan']:
            ...
            for cluster_id in df[cluster_col].unique():
                ...
                if len(cluster_tweets) > 0:
                    # Tomar una muestra del contenido
